...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, dt, level):
        ...
        self.vel.y += self.acc.y * dt  # Simple Euler position update
    ...
    def update_animations(self):
        ...
        if self.frame_count >= animation_length:
            self.frame_count = 0
        self.image = animations[int(self.frame_count)]
        
        if self.facing == 1:
            self.rect = self.image.get_rect(midbottom=self.hitbox.midbottom)
        else:
            self.image = pygame.transform.flip(animations[int(self.frame_count)], True, False)
            self.rect = self.image.get_rect(midbottom=self.hitbox.midbottom)
    ...
    def draw_debug(self, dt, screen):
        if self._debug_info_cooldown_timer <= 0:
            ...
            self.debug_text = self.debug_font.render(
                f"x-Vel: {round(self.vel.x, 2)} y-Vel: {round(self.vel.y, 2)}", True, (255, 125, 125))
        else:
